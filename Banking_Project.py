class Bank: #creating a class in the name of 'Bank'.
    ...
    def __init__(self): #defining init constructor.
        ...
        print("\nYour account is created")    #message for user
    ...
    def Deposit(self):  #defining a function "Deposit".
        ...
        print("\nyour amount is deposited %f" % self.balance) #prints the Deposited amount
       
    def Withdraw(self): #defining a function 'Withdraw'.
        amount = float(input("\nEnter the amount you want to withdraw: "))    #asking user to enter the amount to withdraw
        if(self.balance >= amount): #if-statment checks available balance is greater than & equal to amount which is entered for withdraw
          ...
          print("\nplease collect your cash and remaining balance is %f" % self.balance)  #message for user
        else:   
